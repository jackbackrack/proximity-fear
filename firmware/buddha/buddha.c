...
//Define LED I2C info
#define LED_I2C_SDA 18
#define LED_I2C_SCL 19
...
void i2c_write_byte(uint8_t I2C_addr, uint8_t reg_addr, uint8_t val) {
    ...
    i2c_write_blocking(i2c1, I2C_addr/2, &reg_and_val_arr, 2, false);
#endif
}
...
void IS31FL3236A_mode1(void){
    ...
    for(i=1;i<13;i++){//R LED running
        i2c_write_byte(LED_DRIVER_ADDR, (i*3-1), 0xFF);//set PWM
        i2c_write_byte(LED_DRIVER_ADDR, 0x25, 0x00);//update PWM & congtrol registers
        sleep_ms(100); //100ms
    }

    sleep_ms(500); //keep 0.5s

    for(i=12;i>0;i--){ //G LED running
        i2c_write_byte(LED_DRIVER_ADDR, (i*3-2), 0xFF);//set PWM
        i2c_write_byte(LED_DRIVER_ADDR, 0x25, 0x00);//update PWM & congtrol registers
        sleep_ms(100);//100ms
    }
    ...
    for(i=1;i<13;i++){ //B LED running
        i2c_write_byte(LED_DRIVER_ADDR, (i*3-0), 0xFF);//set PWM
        i2c_write_byte(LED_DRIVER_ADDR, 0x25, 0x00);//update PWM & congtrol registers
        sleep_ms(100);//100ms
    }
...
void pattern_random(uint len) {
    ...
    for (int i = 0; i < len; ++i){
        put_pixel(random_color_scaled(32));
    }
}
